...
class SgsCustodyPortal(http.Controller):
    ...
    def custodian_home(self, token, **kw):
        ...
        clients = request.env["res.partner"].sudo().search([("is_company", "=", True)], order="name")
    ...
    def submit_service(self, token, **post):
        ...
        if post.get("client_id"):
            client = request.env["res.partner"].sudo().browse(int(post["client_id"]))
        vehicle = False
        if post.get("vehicle_id"):
            vehicle = request.env["fleet.vehicle"].sudo().browse(int(post["vehicle_id"]))
        vals = {
